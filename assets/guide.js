...
(function () {
  ...
  function mount({ siteKey }) {
    ...
    let tsId = null, tsToken = "", busy = false;
    ...
    // --- Turnstile: render a managed widget; capture its token via the render callback. ---
    function renderTS() {
    ...
    function resetTS() { tsToken = ""; if (window.turnstile && tsId !== null) { try { window.turnstile.reset(tsId); } catch (e) {} } }

    const openPanel = () => { panel.classList.add("open"); renderTS(); input.focus(); };
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const r = await fetch("/api/chat", {
          ...
          body: JSON.stringify({ message: msg, history: state.history.slice(-8), turnstileToken: token }),
          signal: ctl.signal,
        });
        const data = await r.json().catch(() => ({}));
        thinking.textContent = data.reply || "I'm sorry, I couldn't respond just now. Please try again.";
        ...
        thinking.textContent = (err && err.name === "AbortError")
          ? "That took too long — please try again."
          : "I couldn't reach the guide just now — please try again in a moment.";
      } finally {
        clearTimeout(timer);
        resetTS();      // tokens are single-use; get a fresh one for the next message
        busy = false;
        input.focus();
      }
    });
